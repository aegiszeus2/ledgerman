    ...
    _renderList() {
        ...
        container.innerHTML = `
            ...
            <div class="card" style="margin-bottom:16px">
                <input type="text" id="clientSearch" placeholder="Search clients by name, contact, email, or phone..." value="${Utils.escapeHtml(self._filter)}" style="max-width:400px">
            </div>
            ...
            <div class="modal" style="max-width:600px">
                ...
                <form id="clientModalForm" novalidate>
                    <div class="form-group" style="margin-bottom:12px">
                        ...
                        <input name="name" value="${esc(client ? client.name : '')}" required>
                    </div>
                    <div class="form-group" style="margin-bottom:12px">
                        <label>Contact Person</label>
                        <input name="contactPerson" value="${esc(client ? client.contactPerson : '')}">
                    </div>
                    <div class="form-group" style="margin-bottom:12px">
                        <label>Address</label>
                        <input name="address" value="${esc(client ? client.address : '')}">
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>City</label>
                            <input name="city" value="${esc(client ? client.city : '')}">
                        </div>
                        <div class="form-group">
                            <label>Province</label>
                            <input name="province" value="${esc(client ? client.province : 'Ontario')}">
                        </div>
                        <div class="form-group">
                            <label>Postal Code</label>
                            <input name="postalCode" value="${esc(client ? client.postalCode : '')}">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Phone</label>
                            <input name="phone" value="${esc(client ? client.phone : '')}" type="tel">
                        </div>
                        <div class="form-group">
                            <label>Email</label>
                            <input name="email" value="${esc(client ? client.email : '')}" type="email">
                        </div>
                    </div>
                    <div class="form-actions">
                        <button type="submit" class="btn-primary">${isEdit ? 'Update' : 'Add'} Client</button>
