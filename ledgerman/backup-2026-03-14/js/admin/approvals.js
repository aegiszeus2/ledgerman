...
window.AdminApprovals = {
    ...
    _renderContent() {
        ...
        container.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:16px">
                <h2>Approvals</h2>
                ${pending.length > 1 ? '<button class="btn-primary btn-sm" id="bulkApproveBtn">Approve All (' + pending.length + ')</button>' : ''}
            </div>

            <div class="tabs">
                <button class="tab ${self._tab === 'pending' ? 'active' : ''}" data-tab="pending">
                    Pending ${pending.length > 0 ? '<span style="background:var(--accent);color:#fff;font-size:.7rem;padding:1px 6px;border-radius:10px;margin-left:4px">' + pending.length + '</span>' : ''}
                </button>
                <button class="tab ${self._tab === 'history' ? 'active' : ''}" data-tab="history">
    ...
    _renderPending(contentEl, pending) {
        ...
        contentEl.innerHTML = pending.map(function(sub) {
            ...
            if (sub.rateType === 'flat') {
                amountInfo = 'Flat rate: ' + Utils.formatCurrency(sub.flatAmount || sub.amount);
            } else {
                amountInfo = (parseFloat(sub.hours) || 0) + ' hrs @ ' + Utils.formatCurrency(sub.rate || 0) + '/hr = ' + Utils.formatCurrency((parseFloat(sub.hours) || 0) * (parseFloat(sub.rate) || 0));
            }
            ...
            return '<div class="card" data-sub-id="' + sub.id + '" style="border-left:3px solid var(--warn)">' +
                '<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">' +
                    ...
                    '<div style="display:flex;gap:8px;align-items:flex-start">' +
                        '<button class="btn-primary btn-sm approve-btn" data-id="' + sub.id + '">Approve</button>' +
                        '<button class="btn-danger btn-sm reject-btn" data-id="' + sub.id + '">Reject</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        }).join('');
    ...
    _renderHistory(contentEl, approved, rejected) {
        ...
        contentEl.innerHTML = '<div class="card"><table>' +
            '<thead><tr><th>Date</th><th>Worker</th><th>Project</th><th>Description</th><th class="amount">Amount</th><th>Status</th></tr></thead>' +
            '<tbody>' +
