...
window.AdminExpenses = {
        ...
        container.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:16px">
                ...
                <div style="display:flex;gap:8px;align-items:center">
                    ...
                    <select id="expenseProjectSelect" style="width:auto;min-width:200px">
    ...
    _renderExpenses() {
        ...
        contentEl.innerHTML = `
            <div class="tabs">
                ${tabs.map(function(t) {
                    return '<button class="tab ' + (self._tab === t.key ? 'active' : '') + '" data-tab="' + t.key + '">' + t.label + ' (' + t.count + ')</button>';
                }).join('')}
            </div>

            <div style="margin-bottom:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
                <label style="font-size:.85rem;color:var(--text2)">Filter by category:</label>
                <select id="expCategoryFilter" style="width:auto">
        ...
        contentEl.querySelectorAll('.tab[data-tab]').forEach(function(tab) {
    ...
    _showExpenseForm(type, existing) {
        ...
        const vendorFieldHtml = '<div class="form-group" style="margin-bottom:12px"><label>Vendor</label>' +
            '<select name="vendorSelect" id="vendorSelectField">' +
            ...
            '<label>Vendor Name</label><input name="vendorManual" value="' + esc(existingVendorManual) + '" placeholder="Enter vendor name"></div>';
        ...
        if (isLabor) {
            formFields = `
                ...
                <div class="form-group" style="margin-bottom:12px">
                    <label>Date *</label>
                    <input type="date" name="date" value="${existing ? existing.date || '' : Utils.today()}" required>
                </div>
                <div class="form-group" style="margin-bottom:12px">
                    <label>Subtask</label>
                    <select name="subtaskId">
                ...
                <div class="form-group" style="margin-bottom:12px">
                    ...
                    <input name="description" value="${esc(existing ? existing.description : '')}" required>
                </div>
                ${vendorFieldHtml}
                <div class="form-group" style="margin-bottom:12px">
                    <label>Rate Type</label>
                    <select id="laborRateType">
                ...
                <div id="hourlyFields" class="form-row" ${existing && existing.rateType === 'flat' ? 'style="display:none"' : ''}>
                    <div class="form-group">
                        <label>Hours</label>
                        <input type="number" name="hours" step="0.25" min="0" value="${existing ? existing.hours || '' : ''}">
                    </div>
                    <div class="form-group">
                        <label>Rate ($/hr)</label>
                        <input type="number" name="rate" step="0.01" min="0" value="${existing ? existing.rate || '' : ''}">
                    </div>
                </div>
                <div id="flatFields" class="form-group" style="margin-bottom:12px;${(!existing || existing.rateType !== 'flat') ? 'display:none' : ''}">
                    <label>Flat Amount ($)</label>
                    <input type="number" name="flatAmount" step="0.01" min="0" value="${existing && existing.rateType === 'flat' ? existing.amount || '' : ''}">
                </div>
            `;
        } else {
            formFields = `
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Amount ($) *</label>
                        <input type="number" name="amount" step="0.01" min="0" value="${existing ? existing.amount || '' : ''}" required>
                    </div>
                </div>
                <div class="form-group" style="margin-bottom:12px">
                    <label>Subtask</label>
                ...
                <div class="form-group" style="margin-bottom:12px">
                    <label>Notes</label>
                    <textarea name="notes" rows="2">${esc(existing ? existing.notes : '')}</textarea>
                </div>
            `;
        }
