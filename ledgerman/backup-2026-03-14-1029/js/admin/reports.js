...
window.AdminReports = {
    ...
    _renderReports() {
        ...
        container.innerHTML = `
            ...
            <div class="tabs" style="margin-bottom:16px">
                ${tabs.map(function(t) {
                    return '<button class="tab ' + (self._activeTab === t.id ? 'active' : '') + '" data-tab="' + t.id + '">' + t.label + '</button>';
                }).join('')}
            </div>
    ...
    _downloadCsv(csvContent, reportType) {
        ...
        var filename = 'belfort-' + reportType + '-' + today + '.csv';
    ...
    _exportLaborCsv() {
        ...
        submissions.forEach(function(s) {
            ...
            lines.push(self._csvRow([
                ...
                (parseFloat(s.hoursWorked) || 0).toFixed(2),
                s.status || ''
            ]));
        });
